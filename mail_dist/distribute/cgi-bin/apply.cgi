...
sub reguest {
	...
	unless ( open(PLAN, "$file" ) ) {
		&print_error('<font color="#CC0000">システムエラー</font>', "ファイルが開けません<br>$fileのパーミッションが[606]に設定されているか<br>ご確認してください", 0, 'syserr');
		exit;
	}
	local @line;
	my $csvpath;
	...
	my $index;
	unless ( $csvpath ) {
		&print_error('<font color="#CC0000">システムエラー</font>', "該当するデータがありません。", 0, 'syserr' );
	}
	unless ( open(CSV, "$csvpath" ) ) {
		if ( -e $csvpath ) {
			&print_error('<font color="#CC0000">システムエラー</font>', "ファイルが開けません<br>$csvpathのパーミッションを確認してください。", 0, 'syserr' );
		}
		unless ( open(CSV, ">>$csvpath") ) {
			&print_error('<font color="#CC0000">システムエラー</font>', "ユーザー登録ができません<br>$csvpathのパーミッションが[606]に正しく設定されているか確認してください。", 0, 'syserr' );
		}
		$index = 0;
	}else {
		while( <CSV> ) {
	...
	if ( !$sk || $line[40] ) {
		...
		if ( !$sk ) {
			...
			unless ( $senderror ) {
				...
				&print_error('<font color="#CC0000">システムエラー</font>','登録できません。<br>メール送信プログラムが停止しているか、指定に誤りがあるため<br>メールが送信できませんでした。<br>管理者にお問合せください。', 0, 'syserr');
			}
			# アクセス集計用データ生成
...
sub renewguest {
	...
	my $file = "$myroot$data_dir$log_dir$plan_txt";
	unless ( open(PLAN, "$file" ) ) {
		&print_error('<font color="#CC0000">システムエラー</font>', "ファイルが開けません<br>$fileのパーミッションを確認してください",0,'syserr');
	}
    my @line;
    my $csvpath;
    my $queuepath;
    ...
    unless ( open(CSV, "$csvpath") ) {
        &print_error('<font color="#CC0000">システムエラー</font>', "$csvpathが開けません。",0,'syserr');
		exit;
	}
    my $tmp = "$myroot$data_dir$csv_dir" . $$ . time . '.tmp';
    unless ( open(TMP, ">$tmp") ) {
		&print_error('<font color="#CC0000">システムエラー</font>',"テンポラリーファイルが開けません<br>$csv_dirのパーミッションを確認してください。",0,'syserr');
		exit;
	}
	my $flag  = 0;
	my $agree = 0;
...
sub print_error {
	...
	$type = 'err' if( $type eq 'syserr' ); # システムエラー
