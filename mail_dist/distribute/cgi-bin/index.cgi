...
# ダブルオプトイン
elsif( $mode eq 'opt-in' ){ &make_plan_page( 'plan', 'opt-in' ); }
# ダブルオプトアウト
elsif( $mode eq 'opt-out' ){ &make_plan_page( 'plan', 'opt-out' ); }
# ダブルオプト保存
elsif( $mode eq 'renew_opt' ){ &DoubleOpt::renew(); }
# 空メール
elsif( $mode eq 'kmail' ){ &make_plan_page( 'plan', 'kmail' ); }
# エラーメール
elsif( $mode eq 'bmail' ){ &make_plan_page( 'plan', 'bmail' ); }
# エラーファイル公開
...
sub renew {
	...
	if ( $action eq 'redirect' ) {
		...
		# 解除通知
		$admin_cancel = $param{'notice_cancel'} -0;
		
		# Jcodeライブラリの確認
		if( $utf ){
			unless( &Ctm'jcode_check() ){
				&make_plan_page( "plan", "", "perlライブラリが正しく設置されておりません。<br>文字コードに「UTF-8」を利用するには、<br>一式同梱の『UTF8をご利用の場合』を参照いただいきインストールください。" );
			}
		}
    }
    # 稼動、停止
	...
	while( <PLAN> ) {
		...
		if ( $line[0] eq $id ) {
			if ( $action eq 'bs' ) {
				...
				$line[83] = $ssl;
				$line[84] = $host;
				$line[85] = $admin_cancel;
            }elsif( $action eq 'click_analy' ){
				$line[82] = $param{'addr'}; # Click.pl で生成
			}elsif( $action eq 'opt-in' ){
				$line[87] = $param{'opt-in'} -0;
				$line[88] = $param{'limit'} || 10;
			}elsif( $action eq 'opt-out' ){
				$line[89] = $param{'opt-out'} -0;
				$line[90] = $param{'limit'} || 10;
				$line[91] = $param{'oneclick'} || 0;
			}elsif( $action eq 'kmail'){
				$line[92] = $param{'on1'} -0;
				$line[93] = &the_text( &deltag( $param{'pop1'} ) );
				$line[94] = &the_text( &deltag( $param{'account1'} ) );
				$line[95] = &the_text( &deltag( $param{'pass1'} ) );
				$line[96] = $param{'on2'} -0;
				$line[97] = &the_text( &deltag( $param{'pop2'} ) );
				$line[98] = &the_text( &deltag( $param{'account2'} ) );
				$line[99] = &the_text( &deltag( $param{'pass2'} ) );
			}elsif( $action eq 'bmail'){
				$line[100] = $param{'on'} -0;
				$line[101] = &the_text( &deltag( $param{'pop'} ) );
				$line[102] = &the_text( &deltag( $param{'account'} ) );
				$line[103] = &the_text( &deltag( $param{'pass'} ) );
				$line[104] = &the_text( &deltag( $param{'addr'} ) );
			}elsif( $action eq 'bmail2'){
				$line[105] = $param{'auto'} -0;
				$line[106] = &delspace($param{'err'});
				$line[107] = $param{'err1'}-0;
				$line[108] = $param{'err2'}-0;
			}else {
...
sub body {
    ...
    $n -= 0 if ( $n ne 'r' && $n ne 'c' && $n !~ /^d(\d+)/ && $n ne 'ra' && $n ne 'ca' && $n !~ /^opt/ );
...
sub body_html {
	...
	my $id = &delspace( $param{'id'} -0 );
	my $n = &delspace( $param{'n'} );
    $n -= 0 if ( $n ne 'r' && $n ne 'c' && $n !~ /^d(\d+)/ && $n ne 'ra' && $n ne 'ca' );
...
sub sendtest {
	...
	local ( $queuedir, $queue, $name, $from, $to, $header, $cancel, $footer, $tag_data, $step, $ssl, $host );
	while( <PLAN> ) {
        chomp;
		my ( $index, $_name, $_from, $_to, $file, $_header, $_cancel, $_footer, $_step, $_tag_data, $ssl, $host ) = ( split(/\t/) )[0, 3, 4, 5, 7, 9, 10, 11, 36, 82, 83, 84];
		if ( $index eq $id ) {
			$queuedir = $myroot . $data_dir . $queue_dir;
			$queue  = $queuedir . $file;
			$name   = $_name;
...
sub guest {
	...
	if ( defined $param{'re'} ) {
		...
		$sendck = $sends[2];
		# エラーリスト更新(削除)
		&Bmail::reset( [@line], [$def_mail] ) if( $def_mail ne $csvs[5] );
	}elsif ( defined $param{'de'} ) {
		$target = 'c';
		$sendck = $sends[3];
		# エラーリスト更新(削除)
		&Bmail::reset( [@line], [$def_mail] );
	}elsif( !$reStep ) {
...
sub make_plan_page {
	...
	my $id = $param{'id'} - 0;
	my @line; # データの配列
	my $help = qq|"#" onClick="javascript: return false;" |;
	...
	# ダブルオプトイン
	elsif( $page eq 'opt-in'){ $main_title = 'ダブルオプトイン'; $help = qq|"#" onClick="javascript: return false;" |; }
	# ダブルオプトアウト
	elsif( $page eq 'opt-out'){ $main_title = 'ダブルオプトアウト'; $help = qq|"#" onClick="javascript: return false;" |; }
...
END
		...
		my $ssl_0 = ' selected="selected"' if( $line[83] == 0);
		my $ssl_1 = ' selected="selected"' if( $line[83] == 1);
		my $href_apply = $line[84];
		if( $href_apply eq '' ){
			$href_apply = $applycgi;
			$href_apply =~ s/^https?:\/\///;
		}
		# 解除通知設定
		my $admin_cancel = ' checked="checked"' if( $line[85] );
                                        ...
                                        <table width="100%" border="0" cellspacing="1" cellpadding="3">
                                          ...
                                          <td><font color="#FF0033">※指定がない場合は簡易ページが出力されます。<br>　 詳しくはメニューのヘルプをクリックしてください。</font>
                                            ...
                                            <td bgcolor="#ABDCE5">
											  ...
											  <tr>
                                                <td bgcolor="#E5FDFF" rowspan="2">解除を通知する </td>
                                                <td bgcolor="#FFFFFF"><input type="checkbox" name="notice_cancel" value="1"$admin_cancel>管理者に通知</td>
                                              </tr>
                                              <tr>
                                                <td bgcolor="#FFFFFF">ここにチェックを入れると、「解除時」のメールが管理者のメールアドレスにも送信されます。</td>
                                              </tr>
                                              <tr>
                                              ...
                                              <tr>
                                                <td bgcolor="#FFFFFF">既にメールアドレスを登録されている方から再度登録があった場合の対応を決定します。質問フォーム等、何度も利用するフォームに使用される場合には「許可する」にチェックを入れるとよいでしょう。</td>
                                              </tr>
                                              <tr>
                                                <td bgcolor="#E5FDFF" rowspan="2">SSL設定 </td>
                                                <td bgcolor="#FFFFFF"><select name="ssl">
                                                    <option value="0"$ssl_0>http://</option>
                                                    <option value="1"$ssl_1>https://</option>
                                                  </select>
                                                  <input name="host" type="text" value="$href_apply" size="45"></td>
                                              </tr>
                                              <tr>
                                                <td bgcolor="#FFFFFF">SSLを使用可能\なサーバーにおいてSSL領域に楽メールPROを設置した場合、上記設定を「https://」にしてSSLをご利用ください。<br>
                                                   また、SSL利用時にURLが異なるサーバーの場合には、上記テキストフォーム「apply.cgiのURL」をサーバー環境にあわせて編集してください。<br><br>
                                                   なおこのアドレスは、<br>
                                                   ・「登録/変更/解除用」フォームのサンプルソ\ース<br>
                                                   ・ワンクリック解除リンク<br>
                                                   ・アクセス分析用リンク<br>
                                                   のアクセス先URLとして使用されます。<br><br>
...
END
		...
		my( $source, $source_m, $source_p ) = &make_form( 0, $id, $type, $url, $line[39], $line[58], $line[59], $line[81], @data );
...
END
        ...
        $main_table = <<"END";
                                ...
                                <tr>
                                <td>
								<font color="#FF0000">※確認・完了画面を同一ウィンドウで表\示します</font>
                                <textarea cols="60" rows="20" onFocus="this.select();">
$source</textarea>
                                <br><br><font color="#FF0000">※確認・完了画面をポップアップで表\示します</font>
                                <textarea cols="60" rows="20" onFocus="this.select();">
$source_p</textarea>
                                </td>
                                </tr>
...
<TABLE cellSpacing="0" cellPadding="5" width="500" border="0">
  <TBODY>
    ...
    <TR>
      <TD bgcolor="#FFFFEE">
	ポップアップウインドウにて確認画面・完了画面を表\示している場合、同一ウインドウでの表\示切り替え方法は以下の通りです。<br>
	１）<br>
...
END
		...
		#-------------------------------------
		my $n = &delspace( $param{'n'} );
		$n -= 0 if ( $n ne 'r' && $n ne 'c' && $n !~ /^d(\d+)/ && $n ne 'ra'&& $n ne 'ca' );# ra ca 個別設定
		my $num = $n+1;
		$num = ($n>0)? "第$num回": '登録時';
		$num = '変更時' if( $n eq 'r' );
		$num = '解除時' if( $n eq 'c' );
		$num = '登録時(管理者通知専用)' if( $n eq 'ra' );
		$num = '解除時(管理者通知専用)' if( $n eq 'ca' );
		if ( $n =~ /^d(\d+)/ ) {
		...
		if ( $page eq 'preview' ) {
			...
			# 解除時別設定
			if( $n eq 'c' ){
				$sub_link_r = qq|<a href="$main'indexcgi?md=p&n=ca&id=$id"><font color="#0000FF">&gt;&gt;【管理者通知専用】本文を見る</font></a><br>&nbsp;|;
			}
			if( $n eq 'ca' ){
				$sub_link_r = qq|<a href="$main'indexcgi?md=p&n=c&id=$id"><font color="#0000FF">&gt;&gt;通常の【解除時】本文を見る</font></a><br>&nbsp;|;
				my $conf = ( $ca_conf )? 'する': 'しない';
				$sub_radio = qq|<tr><td bgcolor="#EEEEEE">利用</td><td>$conf</td></tr><tr><td colspan="2">&nbsp;</td></tr>|;
			}
...
END
			...
			# 管理者専用設定
			my $a_conf = ($n eq 'ra' && $ra_conf || $n eq 'ca' && $ca_conf)? 1: 0;
			...
			$body =~ s/<br>/\n/gi;
			$body = &make_text( $body );
			$main_table = &form_mailbody_top( $h, $c, $f, $_text, $_html, $num, $btitle, $body, $n, $id, $a_conf );
		}elsif( $page eq 'html' ){
...
END
        ...
        $main_table = &make_log_table( $id, $line[6], $line[8], $line[36] );
...
END
	...
	# ダブルオプトイン
	elsif( $page eq 'opt-in' ){
		$main_table = &DoubleOpt::pageIn( \@line );
	}
	# ダブルオプトアウト
	elsif( $page eq 'opt-out' ){
		$main_table = &DoubleOpt::pageOut( \@line );
	}
	# 空メール
	elsif( $page eq 'kmail' ){
		( $main_title, $main_table ) = &Kmail::main( \@line );
	}
	# エラーメール
	elsif( $page eq 'bmail' ){
		( $main_title, $main_table ) = &Bmail::main( \@line );
	}
	#--------------------------------------#
	# メニューとメイン部分のテーブルを結合 #
	#--------------------------------------#
...
<tr> 
  ...
  <td width="" bgcolor="#eeffe6"><a href="$indexcgi\?md=mail&id=$id"><font color="#FF9900">登録者へメール送信</font></a></td>
</tr>
END
		}
		$table = <<"END";
                    ...
                    プランの編集 &gt; <strong>$pname</strong>　 $auterun 　　　[ $runlink ]　　 $sendtag</a><hr noshade> <table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr> 
                        <td width="141" height="100%" valign="top"> 
                          <table width="135" border="0" cellpadding="3" cellspacing="0">
                            <tr> 
                              <td  align="left" bgcolor="#b1cca3"><font color="#333333">▼ 基本機能\</font></td>
                            </tr>
                            <tr> 
                              
                              <td width="125" bgcolor="#eeffe6"><a href="$indexcgi\?md=all&id=$id"><font color="#FF9900">詳細</font></a></td>
                            </tr>
                            <tr>
                            ...
                            <tr> 
                              ...
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi\?md=bs&id=$id"><font color="#FF9900">配信元情報</font></a></td>
                            </tr>
                            <tr> 
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=l&id=$id"><font color="#FF9900">配信日程・本文</font></a></td>
                            </tr>
                            <tr> 
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=header&id=$id"><font color="#FF9900">ヘッダー</font></a></td>
                            </tr>
                            <tr> 
                            ...
                            <tr> 
                              ...
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=footer&id=$id"><font color="#FF9900">フッター</font></a></td>
                            </tr>
                            <tr> 
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=redirect&id=$id"><font color="#FF9900">登録設定</font></a></td>
                            </tr>
                            <tr> 
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=form1&id=$id"><font color="#FF9900">登録用フォーム</font></a></td>
                            </tr>
                            <tr> 
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=form2&id=$id"><font color="#FF9900">変更・解除フォーム</font></a></td>
                            </tr>
                            <tr> 
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=ctm_regdisp&id=$id&act=top"><font color="#FF9900">画面カスタマイズ</font></a></td>
                            </tr>
                            <tr> 
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi\?md=g&id=$id"><font color="#FF9900">登録者情報</font></a></td>
                            </tr>
                            $maillink
                          </table><br>
                          <table width="135" border="0" cellpadding="3" cellspacing="0">
                            <tr> 
                              <td align="left" bgcolor="#b1cca3"><font color="#333333">▼ オプション機能\</font></td>
                            </tr>
                            <tr> 
                              
                              <td width="130" bgcolor="#eeffe6"><a href="$indexcgi?md=opt-in&id=$id"><font color="#FF9900">ダブルオプトイン</font></a></td>
                            </tr>
                            <tr> 
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=opt-out&id=$id"><font color="#FF9900">ダブルオプトアウト</font></a></td>
                            </tr>
                             <tr> 
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=copy&id=$id"><font color="#FF9900">コピープラン作成</font></a></td>
                            </tr>
                            <tr>
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=f_magu&id=$id"><font color="#FF9900">まぐまぐ登録機能\</font></a></td>
                            </tr>
                            <tr>
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=click_analy&id=$id"><font color="#FF9900">クリック分析・計測</font></a></td>
                            </tr>
                             <tr> 
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=kmail&act=config&id=$id"><font color="#FF9900">空メール</font></a></td>
                            </tr>
                             <tr> 
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=bmail&act=account&id=$id"><font color="#FF9900">エラー(不達)メール解析</font></a></td>
                            </tr>
                          </table><br>
                          <table width="135" border="0" cellpadding="3" cellspacing="0">
                            <tr> 
                              <td align="left" bgcolor="#b1cca3"><font color="#333333">▼ プランを削除</font></td>
                            </tr>
                            <tr> 
                              
                              <td width="" bgcolor="#eeffe6"><a href="$indexcgi?md=confdel&id=$id"><font color="#FF9900">このプランを削除</font></a></td>
                            </tr>
                          </table>
                        </td>
...
// --&gt;&lt;/script&gt;</textarea>
                                    ...
                                    <BR>
                                    ▼プログラム一式「マニュアル」内 <font color="#0000FF">ご利用サーバーでの制限（特別な場合のみ）.htm</font> をご覧ください。<BR>
                                    <BR>
                                    ...
                                    <br>
                                    ▼プログラム一式「マニュアル」内 <font color="#0000FF">クーロンの設定方法（必要な方のみ）.htm</font> をご覧ください。</TD>
                                </TR>
                              </TBODY>
                            </TABLE></TD>
                        </TR>
				    ...
				    <tr>
					  <td bgcolor="#003580">
					    <table width="100%" border="0" cellspacing="1" cellpadding="3">
						  ...
						  <tr>
						    <td width="80" align="center" bgcolor="#FF9900"><a href="http://www.raku-mail.com/faq/" target="_blank"><font style="font-size:10px"color="#FFFFFF">困ったときは</font></a></td>
						  </tr>
						</table>
					  </td>
					</tr>
...
sub make_schedule {
	...
	local $toYear = ( gmtime(time+(60*60*9)) )[5];
	$toYear += 1900;
...
sub make_log_table {
	...
	my ( $id, $csvfile, $file,$intervals ) = @_;
  ...
  <tr><td>「題名」をクリックすると送信したメールのプレビューを表\示します。<br>
		  ...
		  <font color="#FF0000">※配信後に削除されたステップメールのログ及び「登録者へメール送信」・ダブルオプトイン/アウトで送信されたメールログはリンク表\示されません。</font>
</td>
  </tr>
  <tr><td>&nbsp;</td>
  </tr>
...
sub make_form {
	...
	if ( $type eq 'form1' ) {
		...
		my $source_p = $message. &MF'include( $base, {%FORM} );
		$FORM{'target'} = "";
		$FORM{'javascript'} = "";
		...
		return $source, $source_m, $source_p;
...
END
		return $form_source, $form_source_m, $form_source_p;
	}
}
