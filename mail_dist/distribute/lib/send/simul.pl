...
sub send
{
	...
	while( <PLAN> ) {
		...
		my @plan = split(/\t/);
		if( $plan[0] == $id ){
			&Pub'ssl($plan[83],$plan[84]);
			$err_f = " -f $plan[104]" if( $plan[100] );
		}
	}
	close(PLAN);
	...
	# -f オプション
	my $foption = ( $err_f ne "" )? $err_f: $op_f;
	...
	while(<LIST>){
		...
		$main'param{'id'} = $id;
		&sender( $subject, $sendbody, [@csv], $foption, $r_sleep );
		&log( $id, [@csv], $subject );
