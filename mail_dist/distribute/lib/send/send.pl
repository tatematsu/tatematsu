...
sub csvupload_sender
{
	...
	unless ( open(LOG, ">>$logpath") ) {
		&print_errlog( '', 'ログファイルが開けません(CSVアップロード送信)' );
		return;
		&rename_unlock( $fullpath );
		&format_pro( $process );
	...
	unless ( open(LIST, "$filename") ) {
		&print_errlog( '', '新規登録者リストが開けません(CSVアップロード送信)' );
		return;
		&rename_unlock( $fullpath );
		&format_pro( $process );
	...
	unless ( open(TMPLIST, ">$tmplist") ) {
		&print_errlog( '', "$myroot$data_dir$csv_dir にファイルが作成できません。" );
		return;
		&rename_unlock( $fullpath );
		&format_pro( $process );
	...
	while(<LIST>){
		...
		if ( !$sk ) {
			$senderror = &send( $admin, $sender, $par[5], $subject, $message,"","",[@line] );
			# 配信ログに追加
			unless ( $senderror ) {
				my $date = time;
				open(LOG, ">>$logpath");
