...
sub form
{
	...
	my $optin = ( $def || $param{'optin'} )? ' checked=checked': '';
	my $optout = ( $def || $param{'optout'} )? ' checked=checked': '';
	my $id = $param{'id'} -0;
	my $nid = time . $$;
                                                        ...
                                                        <td align="left"><table width="490" border="0" cellspacing="0" cellpadding="0">
                                                            <tr>
                                                              <td bgcolor="#ABDCE5"><table width="490" border="0" cellpadding="4" cellspacing="1">
                                                                  ...
                                                                  <tr>
                                                                    <td rowspan="7" bgcolor="#E7FFFF">各種設定</td>
                                                                  ...
                                                                  <tr>
                                                                    <td bgcolor="#FFFFFF"><input name="optin" type="checkbox" id="optin" value="1"$optin>
                                                                      ダブルオプトイン　<input name="optout" type="checkbox" id="optout" value="1"$optout>
                                                                      ダブルオプトアウト</td>
                                                                  </tr>
                                                                  <tr>
...
sub copy
{
	...
	my $optin = $param{'optin'} -0;
	my $optout = $param{'optout'} -0;
	...
	if( $optin ){
		my( $default_file, $ctm_file, $target_file ) = &Ctm'get_path( $id, 'opt-in' );
		my( $_default_file, $_ctm_file, $_target_file ) = &Ctm'get_path( $line[0], 'opt-in' );
		if( -f $ctm_file ){
			$path{$ctm_file} = $_ctm_file;
		}
		
		# 携帯用
		my( $default_file_m, $ctm_file_m, $target_file_m ) = &Ctm'get_path( $id, 'opt-in', 1 );
		my( $_default_file_m, $_ctm_file_m, $_target_file_m ) = &Ctm'get_path( $line[0], 'opt-in', 1 );
		if( -f $ctm_file_m ){
			$path{$ctm_file_m} = $_ctm_file_m;
		}
	}
	...
	if( $optout ){
		my( $default_file, $ctm_file, $target_file ) = &Ctm'get_path( $id, 'opt-out' );
		my( $_default_file, $_ctm_file, $_target_file ) = &Ctm'get_path( $line[0], 'opt-out' );
		if( -f $ctm_file ){
			$path{$ctm_file} = $_ctm_file;
		}
		
		# 携帯用
		my( $default_file_m, $ctm_file_m, $target_file_m ) = &Ctm'get_path( $id, 'opt-out', 1 );
		my( $_default_file_m, $_ctm_file_m, $_target_file_m ) = &Ctm'get_path( $line[0], 'opt-out', 1 );
		if( -f $ctm_file_m ){
			$path{$ctm_file_m} = $_ctm_file_m;
		}
	}
