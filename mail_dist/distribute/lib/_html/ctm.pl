...
sub get_path
{
	...
	if( $mobile ){
		...
		$default_file = $default_dir . 'opt-in_m.pl' if( $type eq 'opt-in' );
		$default_file = $default_dir . 'opt-out_m.pl' if( $type eq 'opt-out' );
		...
		$ctm_file = $ctm_dir . $id . '_' . 'opt-in_m.pl' if( $type eq 'opt-in' );
		$ctm_file = $ctm_dir . $id . '_' . 'opt-out_m.pl' if( $type eq 'opt-out' );
	}else{
		...
		$default_file = $default_dir . 'opt-in.pl' if( $type eq 'opt-in' );
		$default_file = $default_dir . 'opt-out.pl' if( $type eq 'opt-out' );
		...
		$ctm_file = $ctm_dir . $id . '_' . 'opt-in.pl' if( $type eq 'opt-in' );
		$ctm_file = $ctm_dir . $id . '_' . 'opt-out.pl' if( $type eq 'opt-out' );
	}
	my $target_file;
